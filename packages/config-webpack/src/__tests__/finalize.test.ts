...
describe('finalize', () => {
    test('can receive a fully resolved webpack config and modify it', async () => {
        ...
        const projectSettings = await readProjectSettings(options);
        const withFinalize = {
            ...projectSettings,
            build: {
    ...
    test('can modify babel config', async () => {
        const finalize = vi.fn((config: TransformOptions):TransformOptions => ({...config, comments: false}));
        const options = {...BUILD_CMD, cwd: currentDirectory};
        const projectSettings = await readProjectSettings(options);
        const withFinalize = {
            ...projectSettings,
        ...
        expect(finalize).toHaveBeenCalled();
        expect(typeof finalize.mock.calls[0][0]).toBe('object');
        expect(typeof (finalize.mock.calls[0][0] as {presets: object}).presets).toBe('object');
    });
});
