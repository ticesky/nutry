...
import {readProjectSettings, CommandInput} from '@nutry/settings';
...
test('can specify a custom webpack entry descriptor', async () => {
    ...
    const config = await createWebpackConfig(context);
    expect((config.entry as Record<string, {filename: string}>)['service-worker'].filename).toBe('service-worker.js');
});
