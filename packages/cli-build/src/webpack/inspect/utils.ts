...
export type Check<T> = (configValue: T, helpers: CheckHelper, options: InspectOptions) => Promise<boolean> | boolean;
...
export const run = async (processors: Array<RuleProcessor<any>>, options: InspectOptions): Promise<void> => {
    const results = await pReduce(
        ...
        async (results, processor) => {
            const normalized = normalizeRuleConfig(processor.config, processor.defaultConfigValue);
            ...
            const result = await processor.check(normalized[1], helpers, options);
