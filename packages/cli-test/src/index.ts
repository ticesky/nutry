...
import {JestConfigOptions, getJestConfig} from '@nutry/config-jest';
import {readProjectSettings, strictCheckRequiredDependency, TestCommandLineArgs} from '@nutry/settings';
...
const resolveJestConfig = async (jestConfigOptions: JestConfigOptions): Promise<string> => {
    ...
    try {
        ...
        const {default: jestConfig} = await import(pathToFileURL(jestConfigFile).toString()) as {default: object};

        if ('preset' in jestConfig) {
            // 如果用户的配置里有`preset`，那它应该已经声明了基于`@nutry/config-jest`来配置，直接返回就行
